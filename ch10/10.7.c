/* deszcz.c -   znajduje sumy roczne, roczna srednia oraz srednia miesieczna dla danych
                o opadach z kilku lat */
...
int main(void){
    ...
    {   // dla każdego roku sumuj opady dla kazdego miesiaca
        for(miesiac = 0, podsuma = 0; miesiac < MIESIACE; miesiac++)
            podsuma += deszcz[rok][miesiac];
        printf("%5d %12.1f\n", 2000 + rok, podsuma);
        suma += podsuma;        // suma wszystkich lat
    }
    printf("\nRoczna srednia wynosi: %.lf cale\n\n", suma/LATA);
