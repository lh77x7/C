...
int main(void){
    ...
    printf("Aby zakonczyc, wcisniej [enter] na poczatku wiersza.\n");
    // licznik (0 - 100)
    // bibl.tytul[0] != '\0' - pierwsze slowo tytulu != '\0'
    // bibl.tytul != NULL - nie jest pustym wyrazem = ' '
    while(licznik < MAXKS && gets(bibl[licznik].tytul) != NULL && bibl[licznik].tytul[0] != '\0')
    {
        ...
        while (getchar() != '\n')
            continue;   // czyść wiersz wejsciowy
        if(licznik < MAXKS)
