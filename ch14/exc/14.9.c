// rozwiazanie zadania 9 z rodzialu 14
...
int main(void){
    ...
    {
        fprintf(stderr, "Could not open file 'flights.dat'.\n");
        exit(EXIT_FAILURE);
    }
    ...
    if ((fp = fopen("flights.dat", "wb")) == NULL)
	{
		fprintf(stderr, "Could not open file 'flights.dat' for write.\n");
		exit(EXIT_FAILURE);
	}

	if (fwrite(flights, flight_size, FLIGHTS, fp) != FLIGHTS)
	{
		fprintf(stderr, "Error writing to file 'flights'.\n");
	}
